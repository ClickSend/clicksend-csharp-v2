/*
 * SMS
 *
 * You can use the SMS endpoints to send and manage SMS, check the cost of sending SMS, view message history, and more. Integrate them into your system as part of your workflow.  Relevant guides: <a href=\"https://help.clicksend.com/article/h474eseq3a-how-many-characters-can-i-send-in-an-sms\" target=\"_blank\">How many characters can I send in an SMS?</a>, <a href=\"https://help.clicksend.com/article/1hd33n2xns-sms-status-delivery-receipt-expired\" target=\"_blank\">SMS status delivery receipt expired</a>, <a href=\"https://help.clicksend.com/en/articles/42195-unicode-vs-plain-text\" target=\"_blank\">Do you support non-English characters</a>, and <a href=\"https://help.clicksend.com/category/mfdctha7f0-country-specific-features-and-restrictions\" target=\"_blank\">Country Specific Features and Restrictions</a>. <div class=\"info-box\">   <h4><i class=\"fas fa-info-circle\"></i> Note :</h4>   ClickSend is pausing SMS messages containing URLs for new customers. Please remove links from messages to maintain uninterrupted delivery, or <a href=\"https://www.clicksend.com/gb/help/\" target=\"_blank\">contact support</a> to apply for approval for URL messaging. </div>
 *
 * The version of the OpenAPI document: 1.0.0
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */

using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using Xunit;
using Microsoft.Extensions.DependencyInjection;
using ClickSend.Api;
using ClickSend.Model;


/* *********************************************************************************
*              Follow these manual steps to construct tests.
*              This file will not be overwritten.
*  *********************************************************************************
* 1. Navigate to ApiTests.Base.cs and ensure any tokens are being created correctly.
*    Take care not to commit credentials to any repository.
*
* 2. Mocking is coordinated by ApiTestsBase#AddApiHttpClients.
*    To mock the client, use the generic AddApiHttpClients.
*    To mock the server, change the client's BaseAddress.
*
* 3. Locate the test you want below
*      - remove the skip property from the Fact attribute
*      - set the value of any variables if necessary
*
* 4. Run the tests and ensure they work.
*
*/


namespace ClickSend.Test.Api
{
    /// <summary>
    ///  Class for testing SmsCampaignsApi
    /// </summary>
    public sealed class SmsCampaignsApiTests : ApiTestsBase
    {
        private readonly ISmsCampaignsApi _instance;

        public SmsCampaignsApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<ISmsCampaignsApi>();
        }

        /// <summary>
        /// Test CalculateSmsCampaignPrice
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task CalculateSmsCampaignPriceAsyncTest()
        {
            Client.Option<string> contentType = default!;
            Client.Option<CalculateSmsCampaignPriceRequest> calculateSmsCampaignPriceRequest = default!;
            var response = await _instance.CalculateSmsCampaignPriceAsync(contentType, calculateSmsCampaignPriceRequest);
            var model = response.Ok();
            Assert.IsType<CalculateSmsCampaignPrice>(model);
        }

        /// <summary>
        /// Test CancelSmsCampaign
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task CancelSmsCampaignAsyncTest()
        {
            string smsCampaignId = default!;
            Client.Option<string> contentType = default!;
            var response = await _instance.CancelSmsCampaignAsync(smsCampaignId, contentType);
            var model = response.Ok();
            Assert.IsType<CancelSmsCampaign>(model);
        }

        /// <summary>
        /// Test SendSmsCampaign
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task SendSmsCampaignAsyncTest()
        {
            Client.Option<string> contentType = default!;
            Client.Option<SendSmsCampaignRequest> sendSmsCampaignRequest = default!;
            var response = await _instance.SendSmsCampaignAsync(contentType, sendSmsCampaignRequest);
            var model = response.Ok();
            Assert.IsType<SendSmsCampaign>(model);
        }

        /// <summary>
        /// Test UpdateSmsCampaign
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task UpdateSmsCampaignAsyncTest()
        {
            string smsCampaignId = default!;
            Client.Option<string> contentType = default!;
            Client.Option<CalculateSmsCampaignPriceRequest> calculateSmsCampaignPriceRequest = default!;
            var response = await _instance.UpdateSmsCampaignAsync(smsCampaignId, contentType, calculateSmsCampaignPriceRequest);
            var model = response.Ok();
            Assert.IsType<UpdateSmsCampaign>(model);
        }

        /// <summary>
        /// Test ViewSmsCampaigns
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task ViewSmsCampaignsAsyncTest()
        {
            Client.Option<string> contentType = default!;
            Client.Option<int> page = default!;
            Client.Option<int> limit = default!;
            Client.Option<string> q = default!;
            Client.Option<string> orderBy = default!;
            Client.Option<int> dateFrom = default!;
            Client.Option<int> dateTo = default!;
            var response = await _instance.ViewSmsCampaignsAsync(contentType, page, limit, q, orderBy, dateFrom, dateTo);
            var model = response.Ok();
            Assert.IsType<ViewSmsCampaigns>(model);
        }

        /// <summary>
        /// Test ViewSpecificSmsCampaign
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task ViewSpecificSmsCampaignAsyncTest()
        {
            string smsCampaignId = default!;
            Client.Option<string> contentType = default!;
            var response = await _instance.ViewSpecificSmsCampaignAsync(smsCampaignId, contentType);
            var model = response.Ok();
            Assert.IsType<ViewSpecificSmsCampaign>(model);
        }
    }
}
