...
{
    /// <summary>
    ///  Class for testing MessageDeliveryApi
    /// </summary>
    public sealed class MessageDeliveryApiTests : ApiTestsBase
    {
        private readonly IMessageDeliveryApi _instance;

        public MessageDeliveryApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<IMessageDeliveryApi>();
        }
        ...
        public async Task CreateDeliveryIssueAsyncTest()
        {
            Client.Option<string> contentType = default!;
            Client.Option<CreateDeliveryIssueRequest> createDeliveryIssueRequest = default!;
            var response = await _instance.CreateDeliveryIssueAsync(contentType, createDeliveryIssueRequest);
            var model = response.Ok();
            Assert.IsType<CreateDeliveryIssue>(model);
        }
        ...
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task GetAllDeliveryIssuesAsyncTest()
        {
            Client.Option<string> contentType = default!;
            var response = await _instance.GetAllDeliveryIssuesAsync(contentType);
            var model = response.Ok();
            Assert.IsType<GetAllDeliveryIssues>(model);
        }
    }
}
